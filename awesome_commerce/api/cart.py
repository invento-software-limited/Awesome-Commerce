...
def calculate_taxes_and_totals(quotation=None, cart_items=None):
    ...
    default_tax_template = frappe.db.exists('Sales Taxes and Charges Template', {'is_default': 1})
    ...
    if frappe.session.user == "Guest":
        ...
        # Apply shipping rule for guest (if exists)
        if shipping_rule:
        ...
        # Skip tax calculations if no default template
                ...
                if tax_row.charge_type == "On Net Total":
                    ...
                    if tax_row.included_in_print_rate:
                        ...
                        order_summary.append({
                            ...
                            "tax_amount": frappe.utils.fmt_money(included_tax, currency=default_currency),
                            "included_in_price": 1
                        })
                    else:
                        excluded_tax = total_price * tax_rate
                        total_price += excluded_tax
                        order_summary.append({
                            "description": tax_row.description,
                            "tax_amount": frappe.utils.fmt_money(excluded_tax, currency=default_currency),
                            "included_in_price": 0
                        })
        ...
        # Only apply taxes if default template exists
        if default_tax_template:
            default_tax_template = frappe.get_cached_doc('Sales Taxes and Charges Template', {'is_default': 1})
            for tax_row in default_tax_template.taxes:
                tax_row_dict = {
